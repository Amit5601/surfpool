...
pub async fn start_studio_and_scenario_server(
    ...
    if std::env::var("SURFPOOL_STUDIO_HOST").is_ok()
        && std::env::var("SURFPOOL_PUBLIC_STUDIO_URL")
            .map_or(true, |url| url.is_empty() || url != url.trim())
    {
        panic!(
            "Configuration Error: SURFPOOL_PUBLIC_STUDIO_URL must be set, non-empty, and contain no leading/trailing whitespace when SURFPOOL_STUDIO_HOST is provided."
        );
    }
